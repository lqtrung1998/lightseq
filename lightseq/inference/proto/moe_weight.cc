...
void MoeWeight<OpType_>::proto_get_model_config(
...
*/
template <OperationType OpType_>
std::string MoeWeight<OpType_>::proto_parse_enc_wei(
...
*/
template <OperationType OpType_>
std::string MoeWeight<OpType_>::proto_parse_dec_wei(
    const Transformer &transformer) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;
...
void MoeWeight<OpType_>::hdf5_get_model_config(hid_t hdf5_file,
  ...
  read_hdf5_dataset_scalar(hdf5_file, "model_conf/expert_num_encoder", H5T_NATIVE_INT,
                           &_expert_num_encoder);

  read_hdf5_dataset_scalar(hdf5_file, "model_conf/expert_num_decoder", H5T_NATIVE_INT,
                           &_expert_num_decoder);

  read_hdf5_dataset_scalar(hdf5_file, "model_conf/moe_topk_encoder", H5T_NATIVE_INT,
                           &_moe_topk_encoder);

  read_hdf5_dataset_scalar(hdf5_file, "model_conf/moe_topk_decoder", H5T_NATIVE_INT,
                           &_moe_topk_decoder);

  _moe_layers_encoder = std::vector<bool>(_n_enc_layer, false);
  ...
  for (int &moe_layer_id:moe_list){
    _moe_layers_encoder[moe_layer_id] = true;
  }

  _moe_layers_decoder = std::vector<bool>(_n_dec_layer, false);
  moe_list= read_hdf5_dataset_data_int(
        hdf5_file, "model_conf/moe_list_decoder", H5T_NATIVE_INT);
  _n_moelayer_decoder = moe_list.size();
  for (int &moe_layer_id:moe_list){
    _moe_layers_decoder[moe_layer_id] = true;
  }                       
}
...
void MoeWeight<OpType_>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  ...
  if (_n_moelayer_encoder){
    value_size += (_hidden_size * _inner_size + _inner_size + _hidden_size * _inner_size +
       _hidden_size) * (_expert_num_encoder - 1) * _n_moelayer_encoder;
  }
  std::vector<int> offset;
  ...
  for (int layer_id = 0; layer_id < _n_enc_layer; ++layer_id) {
    ...
    read_hdf5_dataset_data(
        ...
        value.data() + idx,
        [=](int size) { return size != _hidden_size * _inner_size && size != _hidden_size * _inner_size * _expert_num_encoder; },
        "Wrong ffn_first_kernel_size !");
    if (_moe_layers_encoder[layer_id]){
    ...
    read_hdf5_dataset_data(
        ...
        value.data() + idx, [=](int size) { return size != _inner_size && size != _inner_size * _expert_num_encoder; },
    ...
    read_hdf5_dataset_data(
        ...
        value.data() + idx, [=](int size) { return size != _hidden_size && size != _hidden_size * _expert_num_encoder; },
...
void MoeWeight<OpType_>::hdf5_parse_dec_wei(hid_t hdf5_file) {
  ...
  if (_n_moelayer_decoder){
    value_size += (_hidden_size * _inner_size + _inner_size + _hidden_size * _inner_size +
       _hidden_size) * (_expert_num_decoder - 1) * _n_moelayer_decoder;
  }
  std::vector<int> offset;
  std::vector<float> value(value_size);
  std::cout << "loading " << value_size * sizeof(OpType_) / (1024 * 1024)
  ...
  for (int layer_id = 0; layer_id < _n_dec_layer; ++layer_id) {
    ...
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/ffn_first_kernel", H5T_NATIVE_FLOAT,
        value.data() + idx,
        [=](int size) { return size != _hidden_size * _inner_size && size != _hidden_size * _inner_size * _expert_num_decoder; },
        "Wrong ffn_first_kernel_size !");
    if (_moe_layers_decoder[layer_id]){
    ...
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/ffn_first_bias", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _inner_size && size != _inner_size * _expert_num_decoder; },
        "Wrong ffn_first_bias_size !");
    if (_moe_layers_decoder[layer_id]){
    ...
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/ffn_second_bias", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _hidden_size && size != _hidden_size * _expert_num_decoder; },
        "Wrong ffn_second_bias_size !");
    if (_moe_layers_decoder[layer_id]){
